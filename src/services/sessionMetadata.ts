...
const TOOL_DEFINITION = {
  ...
  function: {
    ...
    description:
      "Generate a short session title, warm summary, and confidence score (0-1).",
    parameters: {
      type: "object",
      ...
      properties: {
        title: {
          type: "string",
          description:
            "3-6 word evocative title capturing the theme of the conversation.",
        },
        summary: {
          type: "string",
        ...
        confidence: {
          ...
          maximum: 1,
          description:
            "Model confidence (0-1) that the title reflects the conversation.",
        },
      },
    },
  },
...
Return thoughtful, caring language with a concise title and summary.`;
  ...
  const body = {
    ...
    tool_choice: {
      type: "function",
      function: { name: "emit_session_metadata" },
    },
  };
  ...
  try {
    ...
    if (error instanceof DOMException && error.name === "AbortError") {
      console.warn("[sessionMetadata] OpenAI request timed out");
    } else {
      console.warn("[sessionMetadata] Falling back to heuristic title", error);
    }
    const fallbackSource =
      messages.find((msg) => msg.kind !== "bot") ?? messages[0];
